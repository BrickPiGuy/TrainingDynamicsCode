...
$config = Join-Path $repoRoot "experiment\configs\followup_cpu_rapid_20m_6s.yaml"
$runDir = Join-Path $repoRoot "experiment\runs\followup_cpu_rapid_20m_6s"

if (-not (Test-Path $python)) {
    throw "Virtual environment not found. Run scripts\setup_windows_xpu.ps1 first."
}

Write-Host "Running rapid CPU follow-up profile (6 seeds)"
